...
void RadioCommand::run()
{
    ...
    while (1)
    {
        ...
        if ((last_command_tick.ticks() > 0) && ((Tick::now() - last_command_tick).ticks() > RADIO_COMMAND_FAILSAFE_TIMEOUT))
        {
        ...
        if (((Tick::now() - last_command_tick).ticks() >= 40) && ((Tick::now() - tx_tick).ticks() >= 1000))
        {
            if (rssi > -20)
            {
